...
void UPDI::reset_target(bool reset)
{
...
uint8_t UPDI::spi_transaction(uint8_t a, uint8_t b, uint8_t c, uint8_t d)
{
    // SPI_LOG(a);
    // SPI_LOG(b);
    // SPI_LOG(c);
...
void UPDI::flash(uint8_t hilo, unsigned int addr, uint8_t data)
{
...
void UPDI::commit(unsigned int addr)
{
    // spi_transaction(0x4C, (addr >> 8) & 0xFF, addr & 0xFF, 0);
}

void UPDI::write_flash_pages(int length)
{
    // int x = 0;
...
void UPDI::write_eeprom_chunk(unsigned int start, unsigned int length)
{
...
uint8_t UPDI::flash_read(uint8_t hilo, unsigned int addr)
{
