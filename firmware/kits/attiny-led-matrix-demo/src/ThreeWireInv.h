#pragma once

#include <util/delay.h>
#include "ports.h"

//ThreeWire command Read/Write flag
const uint8_t THREEWIRE_READFLAG = 0x01;

class ThreeWireInv
{
public:
    ThreeWireInv(uint8_t ioPin, uint8_t clkPin, uint8_t cePin) : _ioPin(ioPin),
                                                                 _clkPin(clkPin),
                                                                 _cePin(cePin)
    {
    }

    void begin()
    {
        resetPins();
    }

    void end()
    {
        resetPins();
    }

    void beginTransmission(uint8_t command)
    {
        PIN_HIGH(_cePin); // default, not enabled
        PIN_OUTPUT(_cePin);

        PIN_LOW(_clkPin); // default, clock low
        PIN_OUTPUT(_clkPin);

        PIN_OUTPUT(_ioPin);

        PIN_LOW(_cePin); // start the session
        _delay_us(4);    // tCC = 4us

        write(command, (command & THREEWIRE_READFLAG) == THREEWIRE_READFLAG);
    }

    void endTransmission()
    {
        PIN_HIGH(_cePin);
        _delay_us(4); // tCWH = 4us
    }

    void write(uint8_t value, bool isDataRequestCommand = false)
    {
        for (uint8_t bit = 0; bit < 8; bit++)
        {
            if (value & 0x01)
                PIN_HIGH(_ioPin);
            else
                PIN_LOW(_ioPin);
            _delay_us(1); // tDC = 200ns

            // clock up, data is read by DS1302
            PIN_HIGH(_clkPin);
            _delay_us(1); // tCH = 1000ns, tCDH = 800ns

            // for the last bit before a read
            // Set IO line for input before the clock down
            if (bit == 7 && isDataRequestCommand)
            {
                PIN_INPUT(_ioPin);
            }

            PIN_LOW(_clkPin);
            _delay_us(1); // tCL=1000ns, tCDD=800ns

            value >>= 1;
        }

        // PIN_OUTPUT(_ioPin);
    }

    uint8_t read()
    {
        uint8_t value = 0;

        for (uint8_t bit = 0; bit < 8; bit++)
        {
            // first bit is present on io pin, so only clock the other
            // bits
            value |= (PIN_READ(_ioPin) << bit);

            // Clock up, prepare for next
            PIN_HIGH(_clkPin);
            _delay_us(1);

            // Clock down, value is ready after some time.
            PIN_LOW(_clkPin);
            _delay_us(1); // tCL=1000ns, tCDD=800ns
        }

        return value;
    }

private:
    const uint8_t _ioPin;
    const uint8_t _clkPin;
    const uint8_t _cePin;

    void resetPins()
    {
        // just making sure they are in a default low power use state
        // as required state is set when transmissions are started
        // three wire devices have internal pull downs so they will be low
        PIN_INPUT(_clkPin);
        PIN_INPUT(_ioPin);
        PIN_INPUT(_cePin);
    }
};
